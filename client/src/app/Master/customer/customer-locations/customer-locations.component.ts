...
export class CustomerLocationsComponent implements OnInit {
  ...
  constructor(private accountService: AccountService, private fb: FormBuilder,private masterService: MasterService 
  ...
  loadCustomerLoc(customerId) { 
      this.masterService.getCustomerLocation(customerId).subscribe(details => {
        //console.log(details);
  ...
  saveDetails() { 
    ...
    var obj = {
      ...
      name: this.customerLocForm.get('name').value == undefined ? '' : this.customerLocForm.get('name').value.trim(),
   ...
   // this.saveobj = Object.assign({}, obj);
    //console.log(obj);
    this.masterService.saveCustomerLocation(obj).subscribe((result) => {    
      if (result == 1) {
  ...
  clearControls() {
   ...
   onEditCustomerLoc(event,cellId) {
    ...
    this.customerLocForm.get('shortCode').setValue(selectedRowData[0]["shortCode"]);
